...
// API host carries registrations for app-only handlers it never maps. That is why its Program.cs
// turns ValidateOnBuild off. This test keeps the resulting hole measured: the API must be able to
// construct every handler except the ones needing a subsystem it deliberately does not wire up.
// A handler breaking for any other reason shows up here as a new entry.
public sealed class ApiContainerTests
{
    private static readonly string[] SubsystemsNotHostedByTheApi =
    [
        ...
        "Netptune.Core.Services.Ai.IAiSpendService",
    ...
    public ApiContainerTests(NetptuneFixture fixture)
    {
        Fixture = fixture;
    }

    [Fact]
    public void ApiHandlers_ShouldOnlyFailToResolve_ForSubsystemsItDoesNotHost()
    {
        ...
        unresolvable.Should().BeEquivalentTo(SubsystemsNotHostedByTheApi);
    }
