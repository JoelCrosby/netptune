...
public sealed class ActivityMergeWindowJob : BackgroundService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            notified += await FinaliseAsync(unitOfWork, notificationEvents, claimed, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (entries.Count == 0) return notifications;
        ...
        {
            if (!slugsByWorkspace.TryGetValue(entry.WorkspaceId, out var slug)) continue;
            if (!usersByWorkspace.TryGetValue(entry.WorkspaceId, out var allUserIds)) continue;

            if (entry.LastActivityLogId == 0)
            {
            ...
            {
                ancestors = await ActivityLinks.Resolve(unitOfWork, entry.EntityType, entry.EntityId, cancellationToken);
                ancestorsByEntity[(entry.EntityType, entry.EntityId)] = ancestors;
            }

            var link = ActivityLinks.Build(slug, entry.EntityType, entry.EntityId, ancestors);

            notifications.AddRange(recipients.Select(userId => new Notification
            {
                UserId = userId,
                ActivityLogId = entry.LastActivityLogId,
