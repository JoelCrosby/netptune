...
public static class StatusesEndpoints
{
    public static RouteGroupBuilder MapStatusesEndpoints(this RouteGroupBuilder group)
    {
        ...
        group.MapGet("/statuses/{id:int}/usage", GetStatusUsage)
            .WithSummary("Get status usage")
            .WithDescription(
                "Reports what still references a status — tasks, projects, board columns and automation rules — "
                + "so you can tell whether deleting it is safe.")
            .RequireAuthorization(NetptunePermissions.Statuses.Read);
    ...
    private static async Task<Results<Ok<EntityUsageViewModel>, NotFound>> GetStatusUsage(
        IMediator mediator,
        int id,
        CancellationToken cancellationToken)
    {
        var result = await mediator.Send(new GetStatusUsageQuery(id), cancellationToken);
