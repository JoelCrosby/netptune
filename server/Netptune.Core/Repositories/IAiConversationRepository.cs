...
{
    Task<PagedResponse<AiConversationViewModel>> GetPageForUser(string userId, int workspaceId, PageRequest request, CancellationToken cancellationToken = default);
    ...
    Task<List<AiConversation>> GetManyInWorkspace(IReadOnlyCollection<Guid> conversationIds, int workspaceId, CancellationToken cancellationToken = default);
