...
public class UpdateWorkspaceRoleCommandHandlerTests
{
    ...
    {
        Handler = new(UnitOfWork, Identity, PermissionCache, Activity);
    }
    ...
    private static bool LogsRoleChange(
        Action<ActivityOptions<UserRoleActivityMeta>> log,
        string userId,
        WorkspaceRole oldRole,
        WorkspaceRole newRole)
    {
        var options = new ActivityOptions<UserRoleActivityMeta>();
        log(options);

        return options.Type == ActivityType.RoleChanged
            && options.Meta.TargetUserId == userId
            && options.Meta.OldRole == oldRole
            && options.Meta.NewRole == newRole;
    }
}
