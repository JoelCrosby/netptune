...
public static class PublicApiBroadcast
{
    // Credentials have no realtime connection of their own, so every public API write names a source no
    // browser can match. That stops connected clients from filtering the change out as one of their own.
    private const string SourceClientId = "public-api";

    // Writes made through the public API have to reach open clients the same way writes made through the
