...
namespace Netptune.Handlers.Projects.Queries;

public sealed record GetProjectsQuery(PageRequest? Page = null) : IRequest<List<ProjectViewModel>>;
...
public sealed class GetProjectsQueryHandler : IRequestHandler<GetProjectsQuery, List<ProjectViewModel>>
{
    ...
    public ValueTask<List<ProjectViewModel>> Handle(GetProjectsQuery request, CancellationToken cancellationToken)
    {
        ...
        return new ValueTask<List<ProjectViewModel>>(UnitOfWork.Projects.GetProjects(workspaceKey, cancellationToken, request.Page));
    }
}
