-- Grants assistant.use to existing members.
...
-- Member and above, matching the code default; viewers are left without it, since a turn spends
-- the workspace's provider budget.
...
SET permissions = permissions || '["assistant.use"]'::jsonb
WHERE role IN ('member', 'admin', 'owner')
  AND NOT permissions @> '["assistant.use"]'::jsonb;
