...
const reducer = createReducer(
  ...
  on(actions.loadNotifications.init, (state): NotificationsState => ({
    ...state,
    loading: true,
  })),
  ...
  on(actions.markAsRead.success, (state, { id }): NotificationsState => ({
    ...state,
  ...
  on(actions.markAllAsRead.success, (state): NotificationsState => ({
    ...state,
    notifications: state.notifications.map((n) => ({ ...n, isRead: true })),
    unreadCount: 0,
  })),

  on(actions.notificationReceived, (state): NotificationsState => ({
    ...state,
