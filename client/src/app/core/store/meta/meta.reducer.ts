...
const reducer = createReducer(
  ...
  on(actions.loadBuildInfo.init, (state): MetaState => ({
    ...state,
    loading: true,
  })),
  on(actions.loadBuildInfo.fail, (state, { error }): MetaState => ({
    ...state,
    loadingError: error,
  })),
  on(actions.loadBuildInfo.success, (state, { buildInfo }): MetaState => ({
    ...state,
    buildInfo,
  }))
);
